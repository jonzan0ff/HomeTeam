...
struct HomeTeamSettingsView: View {
  ...
  var body: some View {
    ...
    .onChange(of: settingsViewModel.pendingSettingsSection) { _, _ in
      applyPendingSectionIfNeeded()
    }
  }
    ...
    VStack(alignment: .leading, spacing: 14) {
      ...
      LazyVGrid(columns: serviceColumns, alignment: .leading, spacing: 8) {
        ForEach(settingsViewModel.sortedStreamingProviders) { provider in
          ...
          .accessibilityIdentifier("settings.streaming.toggle.\(provider.id)")
        }
      }
  ...
  private var aboutSection: some View {
    VStack(alignment: .leading, spacing: 14) {
      Text("About")
        .font(.title2.weight(.bold))
        .accessibilityIdentifier(HomeTeamSettingsSection.about.headingAccessibilityIdentifier)

      Text("Test build version")
        .font(.subheadline.weight(.semibold))

      Text(AppTestVersion.displayString)
        .font(.system(.title3, design: .monospaced))
        .textSelection(.enabled)
        .accessibilityIdentifier("settings.about.testVersion")

      Text("This label is updated in AppTestVersion.swift for each test handoff. It is not the Xcode bundle version.")
        .font(.caption)
        .foregroundStyle(.secondary)
