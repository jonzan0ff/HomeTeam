...
enum TeamCatalog {
  ...
  /// Teams shown in the widget configuration picker. Always includes the full catalog (favorites and recents ordered first) so the macOS configuration UI never gets an empty list when favorites fail to load in the extension process.
