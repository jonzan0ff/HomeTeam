...
- Notarytool keychain profile is named `jonzan0ff-notary` — shared across all four Mac apps (HomeTeam, What to Watch, Dorothy, Print Status). The name is a label; it stores Apple ID + app-specific password credentials that work for any app under the same team.
